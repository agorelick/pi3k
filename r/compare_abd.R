...
get_side_plot <- function(fa2, isoforms) { 
    cols <- rev(c('black',brewer.pal(9, name='RdBu')[c(3,5,9)]))
    names(cols) <- c('Identical','Not-identical, same type','Not-identical, different type','Gap')
    n <- length(isoforms)
    ...
    d[,c('sametype'):=NULL]
    d[,comparison:=paste0(v1,':',v2)]
    d$comparison <- factor(d$comparison, levels=rev(d$comparison))

    d2 <- melt(d, id.vars=c('comparison','v1','v2','n1','n2','aligned'))
    ...
    place_label <- function(x) {
        x$label <- round(100*x$value / x$aligned, 1)
        x$label_pos <- cumsum(x$value)
        x
    }
    d2 <- d2[,place_label(.SD),by=c('comparison')]

    p <- ggplot(d2, aes(x=comparison, y=value)) +
        scale_y_continuous(expand=c(0,0)) +
        scale_x_discrete(expand=c(0,0)) +
        geom_bar(stat='identity',aes(fill=variable)) + 
        geom_text(aes(label=label, y=label_pos),angle=-90,vjust=1)  +
        coord_flip() +
        ang::theme_ang(base_size=12) +
...
p2 <- get_side_plot(fa2, c(c('PIK3CA','PIK3CB','PIK3CD')))
ggsave('figures/multiple_sequence_alignment_comparison_PI3K_sideplot.pdf',width=4,height=2)
...
## generate plot with global alignment indicating side-chain comparisons
p1 <- plot_pairwise_alignment_sidechain_comparisons(d, fa2, highlighted)
ggsave('figures/multiple_sequence_alignment_comparison_RAS.pdf',width=8,height=5)

p2 <- get_side_plot(fa2, c('HRAS','KRAS','NRAS'))
ggsave('figures/multiple_sequence_alignment_comparison_RAS_sideplot.pdf',width=4,height=2)



# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# plot focal region around alignment for A/B/D
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

fa2 <- fread('processed_data/cobalt_alignment_PI3K_annotated.tsv')
aa <- fread(aa_table)
...
hs <- fread('processed_data/aligned_residue_enrichment_BLOSUM80_blastp_intersect.txt')
